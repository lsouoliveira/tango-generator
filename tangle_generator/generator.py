...
class EqualOnEdgeStrategy(SolverStrategy):
    """
    If the current cell has a equal connnection and
    the current cell is one of the two edge cells and
    the opposite edge cell has a filled celll,
    then fill the current cell with the opposite symbol.
    """
    ...
    def apply_at(self, puzzle: Puzzle, row: int, col: int) -> bool:
        ...
        for connection in connections:
            ...
            if row in [0, 1, 4, 5] and connection.direction() in (
                Direction.UP,
                Direction.DOWN,
            ):
                piece = puzzle.peek(5 - row, col)

                if piece:
                    self._place_opposite_symbol(puzzle, row, col, piece)

                    return True

            if col in [0, 1, 4, 5] and connection.direction() in (
                Direction.LEFT,
                Direction.RIGHT,
            ):
                piece = puzzle.peek(row, 5 - col)
    ...
    def _place_opposite_symbol(self, puzzle: Puzzle, row: int, col: int, piece: Piece):
        opposite_symbol = opposite_piece(piece)
...
def worker(target_pieces, connections, result_queue, stop_event):
    best_result = 9999
    iterations = 0
    ...
    while not stop_event.is_set():
        ...
        filled = puzzle.filled_cells_count()

        if filled == target_pieces:
            stop_event.set()
            result_queue.put((puzzle, iterations))
            break
        elif filled < best_result:
            best_result = filled
